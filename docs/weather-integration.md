...
- **Multiple Languages**: Support for various languages (English, Chinese, Japanese, Spanish, French, German)
...
## Migration from Chinese Weather Service

If migrating from the previous Chinese weather service:

1. **Replace Plugin**: Change `get_weather` to `get_western_weather` in configuration
2. **Update API Key**: Replace QWeather API key with OpenWeatherMap API key
3. **Update Default Location**: Change default location from Chinese city to Western city

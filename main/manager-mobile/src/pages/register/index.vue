...
const sm2PublicKey = computed(() => configStore.config.sm2PublicKey)
...
async function handleRegister() {
  ...
  if (!sm2PublicKey.value) {
    toast.warning('SM2 public key not configured, please contact administrator')
    return
  }
  ...
  try {
    ...
    const registerData = {
      ...
      password: sm2Encrypt(sm2PublicKey.value, `${formData.value.captcha}${formData.value.password}`),
      captchaId: formData.value.captchaId,

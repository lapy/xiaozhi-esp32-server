...
hass_play_music_function_desc = {
    ...
    "function": {
        ...
        "description": "Used when user wants to listen to music or audiobooks, plays corresponding audio on room media player (media_player)",
        "parameters": {
            "type": "object",
            "properties": {
                "media_content_id": {
                    "type": "string",
                    "description": "Can be album name, song name, or artist of music or audiobook, if not specified fill in random",
                },
                "entity_id": {
                    "type": "string",
...
async def handle_hass_play_music(conn, entity_id, media_content_id):
    ...
    if response.status_code == 200:
        return f"Playing music of {media_content_id}"
    else:

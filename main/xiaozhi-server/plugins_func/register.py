...
class ToolType(Enum):
    ...
    CHANGE_SYS_PROMPT = (3, "Modify system prompt, switch role personality or responsibility")
    SYSTEM_CTL = (
        4,
...
class Action(Enum):
    ...
    REQLLM = (3, "Call function then request LLM to generate response")
    RECORD = (4, "Record tool call to dialogue history without calling LLM")
...
def register_function(name, desc, type=None):
    ...
    def decorator(func):
        ...
        logger.bind(tag=TAG).debug(f"Function '{name}' loaded and ready for registration")
        return func

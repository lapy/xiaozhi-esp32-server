    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            if not is_valid_image_file(image_data):
                raise ValueError(
                    "Unsupported file format, please upload valid image file (supports JPEG, PNG, GIF, BMP, TIFF, WEBP formats)"
                )
    ...
    async def handle_get(self, request):
        ...
        try:
            ...
            if vision_explain and len(vision_explain) > 0 and "null" != vision_explain:
                message = (
                    f"MCP Vision interface running normally, visual explanation interface address is: {vision_explain}"
                )
            else:
                message = "MCP Vision interface running abnormally, please open .config.yaml file in data directory, find 【server.vision_explain】, and set the address"

...
class OTAHandler(BaseHandler):
    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            if mqtt_gateway_endpoint:  # If configured non-empty string
                # Try to get device model from request data
                ...
                self.logger.bind(tag=TAG).info(f"Distributed MQTT gateway configuration for device {device_id}")
                
                
            else:  # MQTT gateway not configured, distribute WebSocket
                return_json["websocket"] = {
                    "url": self._get_websocket_url(local_ip, port),
                }
                self.logger.bind(tag=TAG).info(f"MQTT gateway not configured, distributed WebSocket configuration for device {device_id}")

...
- **When tool use is required:**
  ...
  4. Detailed standard-calendar questions for dates outside the current context, such as tomorrow's weekday, a specific date, or week-number/date calculations.
...
- **When tool use is not required:**
  1. Information already present in `<context>`, such as the current time, today's date, today's formatted date, or local weather.
...
class ConnectionHandler:
    def __init__(
        ...
        self.is_exiting = False  # Track whether the exit flow is already in progress.
        ...
        self.vad_last_voice_time = 0.0  # Last user speech timestamp in milliseconds.
    ...
    async def _route_message(self, message):
        ...
        # Ignore new messages while the exit flow is already running.
        if self.is_exiting:
            return
            ...
            if not bHasError and len(tool_calls_list) > 0:
                ...
                for future, tool_call_data, tool_input in futures_with_data:
                    try:
                        ...
                        # Return a fallback error response so one failed tool
                        # call does not stall the whole response flow.
                        tool_results.append((
                            ActionResponse(
                                action=Action.ERROR,
                                result="Sorry, the network ran into a problem. Please try again in a moment.",
                            ),
                            tool_call_data
                        ))
                        # Report tool-call failures through the shared helper.

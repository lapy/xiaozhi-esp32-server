...
async def send_mcp_message(conn: "ConnectionHandler", payload: dict):
    ...
    if not conn.features.get("mcp"):
        logger.bind(tag=TAG).warning(
            "The client does not support MCP, so MCP messages cannot be sent"
        )
        return
    ...
    try:
        ...
        logger.bind(tag=TAG).debug(f"Sent MCP message successfully: {message}")
...
async def handle_mcp_message(
    ...
    """Handle MCP messages including initialization, tool lists, and tool-call responses."""
    logger.bind(tag=TAG).debug(f"Handling MCP message: {str(payload)[:100]}")

    if not isinstance(payload, dict):
        logger.bind(tag=TAG).error("MCP message is missing a payload or is malformed")
        return
    ...
    if "result" in payload:
        ...
        # Check for a tool-call response first.
        ...
        if msg_id == 1:  # mcpInitializeID
            ...
            logger.bind(tag=TAG).debug(
                "Initialization finished, starting MCP tool-list request"
            )
            ...
            if isinstance(result, dict) and "tools" in result:
                ...
                logger.bind(tag=TAG).info(
                    f"Client device exposes {len(tools_data)} tools"
                )
                ...
                for tool_data in mcp_client.tools.values():
                    if "description" in tool_data:
                        description = tool_data["description"]
                        # Rewrite names to their sanitized forms.
                        for (
                            sanitized_name,
                            original_name,
                ...
                if next_cursor:
                    ...
                    logger.bind(tag=TAG).debug(
                        "All tools fetched, MCP client is ready"
                    )

                    # Refresh tool caches so MCP tools appear in function lists.
...
async def send_mcp_initialize_message(conn: "ConnectionHandler"):
    ...
    # Generate a token from the server auth key.
...
async def send_mcp_tools_list_request(conn: "ConnectionHandler"):
    """Send the MCP tool-list request."""
    payload = {
        "jsonrpc": "2.0",
        "id": 2,  # mcpToolsListID
        "method": "tools/list",
...
async def send_mcp_tools_list_continue_request(conn: "ConnectionHandler", cursor: str):
    ...
    logger.bind(tag=TAG).info(
        f"Sending MCP tool-list continuation request: {cursor}"
    )
    await send_mcp_message(conn, payload)
...
async def call_mcp_tool(
    ...
    """
    Call a specific tool and wait for its response.
    """

...
class ToolManager:
    """Unified tool manager for every tool type."""
    ...
    def get_function_descriptions(self) -> List[Dict[str, Any]]:
        """Get function descriptions for all tools in OpenAI format."""
    ...
    def has_tool(self, tool_name: str) -> bool:
        """Check whether the named tool exists."""
        tools = self.get_all_tools()
        return tool_name in tools

    def get_tool_type(self, tool_name: str) -> Optional[ToolType]:
        """Get the tool type for a named tool."""
        tools = self.get_all_tools()
    ...
    async def execute_tool(
        ...
        try:
            ...
            # Fetch the matching executor.
            executor = self.executors.get(tool_type)
            if not executor:
                return ActionResponse(
                    action=Action.ERROR,
                    response=f"No executor is registered for tool type {tool_type.value}",
                )
            ...
            self.logger.error(f"Error while executing tool {tool_name}: {e}")

...
class ServerMCPManager:
    ...
    async def initialize_servers(self) -> None:
        ...
        if hasattr(self.conn, "func_handler") and self.conn.func_handler:
            # Refresh tool cache to ensure server-side MCP tools are correctly loaded
    ...
    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        ...
        logger.bind(tag=TAG).info(f"Executing server-side MCP tool {tool_name}, parameters: {arguments}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Throw exception directly when last attempt fails
    ...
    async def cleanup_all(self) -> None:
        ...
        for name, client in list(self.clients.items()):
            try:
                ...
                logger.bind(tag=TAG).error(f"Error closing server-side MCP client {name}: {e}")
        self.clients.clear()

...
class ServerMCPManager:
    ...
    def __init__(self, conn) -> None:
        ...
        if not os.path.exists(self.config_path):
            ...
            logger.bind(tag=TAG).warning(
                "Please check the MCP service config file: data/.mcp_server_settings.json"
            )
    ...
    async def initialize_servers(self) -> None:
        ...
        # Refresh and log the currently supported server-side MCP tools.
        if hasattr(self.conn, "func_handler") and self.conn.func_handler:
            # Refresh tool caches so server-side MCP tools are included.
    ...
    def get_all_tools(self) -> List[Dict[str, Any]]:
        """Get tool function definitions from every service."""
        return self.tools

    def is_mcp_tool(self, tool_name: str) -> bool:
        """Check whether a tool belongs to an MCP service."""
    ...
    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        """Execute a tool call and retry with reconnection on failure."""
        logger.bind(tag=TAG).info(
            f"Executing server-side MCP tool {tool_name}, arguments: {arguments}"
        )
        ...
        # Execute with retry and reconnect behavior.
        for attempt in range(max_retries):
            try:
                return await target_client.call_tool(tool_name, arguments, progress_callback=self.progress_callback)
            except Exception as e:
                # Raise immediately on the final failed attempt.
                if attempt == max_retries - 1:
                    raise

                logger.bind(tag=TAG).warning(
                    f"Tool execution failed for {tool_name} (attempt {attempt+1}/{max_retries}): {e}"
                )

                # Attempt a reconnect before retrying.
                logger.bind(tag=TAG).info(
                ...
                try:
                    ...
                    if client_name in config:
                        ...
                        target_client = client
                        logger.bind(tag=TAG).info(
                            f"Reconnected MCP client successfully: {client_name}"
                        )
                    else:
                        logger.bind(tag=TAG).error(
    ...
    async def cleanup_all(self) -> None:
        ...
        for name, client in list(self.clients.items()):
            try:
                ...
                logger.bind(tag=TAG).info(f"Closed server-side MCP client: {name}")
            except (asyncio.TimeoutError, Exception) as e:
                logger.bind(tag=TAG).error(
                    f"Error while closing server-side MCP client {name}: {e}"
                )
        self.clients.clear()

...
class TTSProvider(TTSProviderBase):
    ...
    def __init__(self, config, delete_audio_file):
        ...
        # Apply percentage-based tuning when configured.
        self._apply_percentage_params(config)

        self.output_file = config.get("output_dir", "tmp/")
        
        logger.debug(
            f"TTS parameters initialized: model={self.model}, voice={self.voice}, format={self.audio_file_type}, speed={self.speed}, api_url={self.api_url}"
        )

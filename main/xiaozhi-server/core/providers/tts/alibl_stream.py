...
class TTSProvider(TTSProviderBase):
    ...
    async def _ensure_connection(self):
        ...
        try:
            ...
            if self.ws and current_time - self.last_active_time < 60:
                # Can reuse connection for continuous conversation within one minute
    ...
    async def start_session(self, session_id):
        ...
        try:
            # Check and clean up previous session monitoring task
            if (
                self._monitor_task is not None
                and isinstance(self._monitor_task, Task)
                and not self._monitor_task.done()
            ):
                logger.bind(tag=TAG).info("Detected incomplete previous session, closing monitoring task...")
                await self.close()
    ...
    async def finish_session(self, session_id):
        ...
        try:
            if self.ws and session_id:
                ...
                # Wait for monitoring task completion
                if self._monitor_task:
                    try:
                        await self._monitor_task
                    except Exception as e:
                        logger.bind(tag=TAG).error(
                            f"Error occurred while waiting for monitoring task completion: {str(e)}"
                        )
                    finally:
                        self._monitor_task = None
    ...
    async def _start_monitor_tts_response(self):
        ...
        try:
            ...
            # Only close connection when there's an exception and not normal end

...
async def sendAudioMessage(conn: "ConnectionHandler", sentenceType, audios, text, sentence_id=None):
    # Skip leftover audio from an older sentence.
...
async def send_tts_message(conn: "ConnectionHandler", state, text=None):
    ...
    if state == "stop":
        # Save the active sentence ID so stale stop messages can be ignored.
        ...
        # Ignore stop messages from older sentences.
        if current_sentence_id != conn.sentence_id:
            return

        # Stop the background audio-sending loop once flow control exists.
        if hasattr(conn, "audio_rate_controller") and conn.audio_rate_controller:

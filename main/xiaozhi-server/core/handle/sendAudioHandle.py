...
def calculate_timestamp_and_sequence(conn, start_time, packet_index, frame_duration=60):
    ...
    if hasattr(conn, "audio_flow_control"):
        ...
        sequence = packet_index  # If no flow control state, use index directly
...
async def sendAudio(conn, audios, frame_duration=60):
    """
    Send single opus packet with flow control support
    Args:
        conn: Connection object
        opus_packet: Single opus data packet
        pre_buffer: Fast audio sending
        frame_duration: Frame duration (milliseconds), matches Opus encoding
    """
    ...
    if isinstance(audios, bytes):
        ...
        if conn.conn_from_mqtt_gateway:
            ...
            # Call common function to send packet with header
            await _send_to_mqtt_gateway(conn, audios, timestamp, sequence)
        else:
            # Send opus data packet directly, no header added
        ...
        # File-type audio uses normal playback
        start_time = time.perf_counter()
        play_position = 0

        # Execute pre-buffering
        ...
        for i in range(pre_buffer_frames):
            if conn.conn_from_mqtt_gateway:
                ...
                # Send pre-buffered packets directly, no header added
        ...
        for i, opus_packet in enumerate(remaining_audios):
            ...
            if conn.conn_from_mqtt_gateway:
                # Calculate timestamp and sequence number (using current packet index to ensure continuity)
    ...
    try:
        ...
        if text.strip().startswith("{") and text.strip().endswith("}"):
            ...
            if isinstance(parsed_data, dict) and "content" in parsed_data:
                # If it's JSON format with speaker info, only display content part

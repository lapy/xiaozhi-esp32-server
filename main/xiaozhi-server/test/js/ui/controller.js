...
class UIController {
    ...
    initEventListeners() {
        ...
        if (cameraBtn) {
            cameraBtn.addEventListener('click', () => {
                ...
                cameraTimer = setTimeout(() => {
                    ...
                    if (!cameraContainer) {
                        log('Camera container not found.', 'warning');
                        return;
                    }
                        ...
                        if (typeof window.startCamera === 'function') {
                            window.startCamera().then(success => {
                                if (success) {
                                    ...
                                    this.addChatMessage('Warning: failed to start the camera. Check the browser permissions.', false);
                                }
                            }).catch(error => {
                                log(`Unexpected camera startup error: ${error.message}`, 'error');
                            });
                        } else {
                            log('startCamera is not defined.', 'warning');
                        }
                    }
                }, 300);
            });
            ...
            recordBtn.addEventListener('click', () => {
                ...
                recordTimer = setTimeout(() => {
                    ...
                    if (audioRecorder.isRecording) {
                        audioRecorder.stop();
                        // Restore the record button to its idle state.
                        recordBtn.classList.remove('recording');
                        recordBtn.querySelector('.btn-text').textContent = 'Record';
                    } else {
                        // Update the button to the recording state.
        ...
        // Close modals when clicking the backdrop, except for explicitly locked dialogs.
        const modals = document.querySelectorAll('.modal');
        modals.forEach(modal => {
            modal.addEventListener('click', (e) => {
                if (e.target === modal) {
                    // These dialogs can only be closed with the X button.
    ...
    updateDialButton(isConnected) {
        ...
        if (cameraBtn && !isConnected) {
            ...
            cameraBtn.title = 'Connect to the server first';
            // Turn the camera off.
            if (typeof window.stopCamera === 'function') {
                window.stopCamera();
            }
        ...
        if (cameraBtn && isConnected) {
            if (window.cameraAvailable) {
                ...
                cameraBtn.title = 'Open or close the camera';
            } else {
                cameraBtn.disabled = true;
            ...
            if (isConnected && microphoneAvailable) {
                ...
                if (!microphoneAvailable) {
                    recordBtn.title = window.isHttpNonLocalhost
                        ? 'Recording is unavailable over non-localhost HTTP. Use text chat instead.'
                        : 'Microphone unavailable';
                } else {
                    recordBtn.title = 'Connect to the server first';
                }
                // Restore the record button to its idle state.
                recordBtn.querySelector('.btn-text').textContent = 'Record';
                recordBtn.classList.remove('recording');
    ...
    updateMicrophoneAvailability(isAvailable, isHttpNonLocalhost) {
        ...
        if (!isAvailable) {
            ...
            // If connected, enable the record button.
            const wsHandler = getWebSocketHandler();
            ...
            app.live2dManager.switchModel(selectedModel)
                .then(success => {
                    if (success) {
                        ...
                        this.addChatMessage('Failed to switch the model.', false);
                    }
                })
                .catch(error => {
                    console.error('Model switch error:', error);
                    this.addChatMessage('An error occurred while switching the model.', false);
                });
        } else {
            this.addChatMessage('The Live2D manager is not initialized yet.', false);
        }
    }
    ...
    startAIChatSession() {
        this.addChatMessage('Connected successfully. Start chatting whenever you are ready. 😊', false);
        // Check microphone availability and show guidance if needed.
        if (!window.microphoneAvailable) {
            if (window.isHttpNonLocalhost) {
                this.addChatMessage('Warning: recording is unavailable over non-localhost HTTP, so only text chat is available.', false);
            } else {
                this.addChatMessage('Warning: the microphone is unavailable. Check permissions and use text chat for now.', false);
            }
        }
        // Start recording only when the microphone is available.
        if (window.microphoneAvailable) {
            const recordBtn = document.getElementById('recordBtn');
            if (recordBtn) {
                recordBtn.click();
        ...
        // Start the camera only when it is available for the current device state.
        if (window.cameraAvailable && typeof window.startCamera === 'function') {
            window.startCamera().then(success => {
                if (success) {
                    ...
                    this.addChatMessage('Warning: failed to start the camera. The browser may have denied access.', false);
                }
            }).catch(error => {
                log(`Unexpected camera startup error: ${error.message}`, 'error');
            });
    ...
    async handleConnect() {
        ...
        if (!otaUrlInput || !otaUrlInput.value) {
            this.addChatMessage('Please enter an OTA server URL.', false);
            return;
        }
        ...
        this.addChatMessage('Connecting to the server...', false);
        ...
        try {
            ...
            const errorMessage = error.message.includes('Cannot set properties of null')
                ? 'Connection failed. Check the device connection.'
                : `Connection failed: ${error.message}`;

            this.addChatMessage(errorMessage, false);

            // Restore the dial button state.
            const dialBtn = document.getElementById('dialBtn');
            if (dialBtn) {
                if (!this.dialBtnDisabled) {
                    dialBtn.disabled = false;

 ...
 * @param {Function} [config.retryFunction] - Retry callback
 */
function makeApiRequest(config) {
  ...
  requestBuilder
    ...
    .fail((err) => {
      console.error(errorMessage || 'Request failed', err);
      if (errorCallback) {
        errorCallback(err);
      }
    })
...
export default {
   ...
   * @param {Object} data - Knowledge base payload
   * @param {Function} callback - Success callback
   * @param {Function} errorCallback - Error callback
   */
   ...
   * @param {string|Array} ids - Knowledge base IDs as a string or array
   * @param {Function} callback - Success callback
   * @param {Function} errorCallback - Error callback
   */
  deleteKnowledgeBases(ids, callback, errorCallback) {
    // Ensure ids are serialized as a comma-separated string.
    ...
    makeApiRequest({
      ...
      callback: callback,
      errorCallback: errorCallback,
      errorMessage: 'Failed to delete knowledge bases',
  ...
  listChunks(datasetId, documentId, params, callback, errorCallback) {
    ...
    // Add keyword search parameters when provided.
    if (params.keywords) {
    ...
    makeApiRequest({
      ...
      callback: callback,
      errorCallback: errorCallback,
      errorMessage: 'Failed to fetch chunk list',
   ...
   * @param {string} datasetId - Knowledge base ID
   * @param {Object} data - Retrieval test payload
   * @param {Function} callback - Success callback
   * @param {Function} errorCallback - Error callback
   */

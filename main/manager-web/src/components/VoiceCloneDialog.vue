...
export default {
    ...
    methods: {
        ...
        async handleFileChange(file) {
            ...
            // Move to step two first so the DOM can render
            this.currentStep = 2;

            // Wait for the DOM update before loading audio
            await this.$nextTick();
        ...
        drawWaveform() {
            const canvas = this.$refs.waveformCanvas;
            if (!canvas) {
                console.error('Canvas does not exist while drawing waveform');
                return;
            }
            ...
            this.waveformData.forEach((value, index) => {
                // Normalize and scale to 80% of the height

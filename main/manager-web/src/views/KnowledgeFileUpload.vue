...
export default {
  ...
  computed: {
    ...
    // Compute slice pagination page count
    slicePageCount() {
      return Math.ceil(this.sliceTotal / this.slicePageSize);
    },
    // Compute visible slice pagination pages (max 3)
    sliceVisiblePages() {
      const pages = [];
      const maxVisible = 3;
  ...
  methods: {
      ...
      KnowledgeBaseAPI.getDocumentList(this.datasetId, params,
        async ({ data }) => {
          ...
          if (data && data.code === 0) {
            ...
            // Fetch the slice count for each document
            await this.fetchSliceCountsForDocuments();
            
            // Automatically start status checks for processing documents
    ...
    startStatusPolling: function () {
      ...
      // Check whether any documents are being processed
      const hasProcessingDocuments = this.fileList.some(document => 
        document.parseStatusCode === 1
      );
      
      if (!hasProcessingDocuments) {
        console.log('No documents are being processed, skipping status polling');
        return;
      }
      ...
      // Run one status check immediately
    ...
    pollDocumentStatus: async function () {
      ...
      if (Date.now() - this.statusPollingStartTime > this.maxStatusPollingTime) {
        console.log('Reached maximum polling time, stopping status polling');
        this.stopStatusPolling();
        return;
      }
      ...
      try {
        ...
        if (response && response.code === 0) {
          ...
          // Check whether any documents are still processing
          const hasProcessingDocuments = updatedFileList.some(document => 
            document.parseStatusCode === 1
          );
          
          if (!hasProcessingDocuments) {
            console.log('All documents finished processing, stopping status polling');
            this.stopStatusPolling();
          }
        }
      } catch (error) {
    ...
    updateDocumentStatuses: function (updatedFileList) {
      ...
      updatedFileList.forEach(updatedDoc => {
        ...
        if (existingDoc && existingDoc.parseStatusCode !== updatedDoc.parseStatusCode) {
          // Update the document when its status changes
          ...
          // If the status becomes completed, refresh the slice count
    ...
    // Fetch slice counts for each document in the list
    ...
    fetchSliceCountForSingleDocument: function (documentId) {
      ...
      const params = {
        page: 1,
        page_size: 1  // Only need the total, so one item per page is enough
      };
      ...
      KnowledgeBaseAPI.listChunks(this.datasetId, documentId, params,
        ({ data }) => {
          if (data && data.code === 0) {
            ...
            // Update the document slice count
            this.$set(document, 'sliceCount', sliceCount);
            // Force the view to refresh
            this.$forceUpdate();
            console.log(`Document ${document.name} slice count updated to:`, sliceCount);
          } else {
            console.warn(`Failed to fetch slice count for document ${document.name}:`, data?.msg);
          }
        },
        (err) => {
          console.warn(`Failed to fetch slice count for document ${document.name}:`, err);
        }
      );
    },
    
    // Smartly detect slice generation state and refresh automatically
    smartRefreshSliceCount: function (documentId) {
      ...
      // Wait 2 seconds before fetching the slice count to give the backend more time
      setTimeout(() => {
    ...
    showUploadDialog: function () {
      ...
      this.selectedFilesList = []; // Clear the selected file list
    ...
    handleFileChange: function (file, fileList) {
      ...
      if (!isLt10M) {
        this.$message.error('File size cannot exceed 10 MB!');
        return;
      }
    ...
    beforeUpload: function (file) {
      ...
      // Store the file in uploadForm
      this.uploadForm.file = file;
      return false; // Prevent auto upload and use custom upload logic
    },
      ...
      const uploadPromises = this.selectedFilesList.map(file => {
        return new Promise((resolve, reject) => {
          ...
          KnowledgeBaseAPI.uploadDocument(this.datasetId, formData,
            ...
            (err) => {
              // Handle backend error messages in the error callback
              if (err && err.data) {
    ...
    // Single-file upload (kept for compatibility)
        ...
        KnowledgeBaseAPI.parseDocument(this.datasetId, row.id,
          ({ data }) => {
            if (data && data.code === 0) {
              ...
              // Update the document state to processing immediately
      ...
      KnowledgeBaseAPI.listChunks(this.datasetId, this.currentDocumentId, params,
        ({ data }) => {
          ...
          if (data && data.code === 0) {
            ...
            this.$message.error(data?.msg || 'Failed to fetch slice list');
            this.sliceList = [];
            this.sliceTotal = 0;
          }
        },
        (err) => {
          ...
          // Handle backend error messages in the error callback
          if (err && err.data) {
            this.$message.error(err.data.msg || err.msg || 'Failed to fetch slice list');
          } else {
            this.$message.error('Failed to fetch slice list');
          }
          console.error('Failed to fetch slice list:', err);
          this.sliceList = [];
          this.sliceTotal = 0;
        }
      );
...
.main-wrapper {
  // Top 63px, bottom 35px, search 58px
  ...
  .slice-card {
    ...
    .slice-card-content {
      ...
      .content-text {
        ...
        /* Ensure text renders normally, including spaces and line breaks */
        white-space: pre-wrap;

...
export default {
  ...
  mounted() {
    ...
    if (this.isCDNEnabled) {
      // Add global hotkey Alt+C to show cache viewer
      ...
      console.info(
        'Press Alt+C key combination or run checkCDNCacheStatus() in console to view CDN cache status'
      );
  ...
  beforeDestroy() {
    // Only need to remove event listener when CDN is enabled
    if (this.isCDNEnabled) {
  ...
  methods: {
    ...
    isMobileDevice() {
      // Function to detect if it is a mobile device
